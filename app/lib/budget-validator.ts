...
export function evaluateBudgetMath(params: {
  ...
  const { targetFundingUsdc, docText, milestones = [] } = params;
  ...
  // 2. Extract monetary amounts from uploaded document texts
  ...
  // 3. Line-Item Category Classification
  ...
  const categoryBreakdown: BudgetCategoryAllocation[] = (Object.keys(categories) as Array<BudgetCategoryAllocation["category"]>).map((cat) => ({
    category: cat,
  ...
  // 4. Budget Category Sanity Rules
